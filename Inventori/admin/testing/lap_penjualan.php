...
$pdf->Cell(1.5, 0.9, 'ID Spl', 1, 0, 'C');
$pdf->Cell(3.5, 0.9, 'Nama Spl', 1, 0, 'C');
$pdf->Cell(1.5, 0.9, 'No SJ', 1, 0, 'C');
$pdf->Cell(1, 0.9, 'Rak', 1, 0, 'C');
$pdf->Cell(2, 0.9, 'Jenis Brg', 1, 0, 'C');
$pdf->Cell(2, 0.9, 'Satuan', 1, 0, 'C');
$pdf->Cell(1, 0.9, 'Qty', 1, 0, 'C');
$pdf->Cell(2.2, 0.9, 'Harga Beli', 1, 0, 'C');
$pdf->Cell(2.2, 0.9, 'Harga Jual', 1, 0, 'C');
$pdf->Cell(3, 0.9, 'Tgl Masuk', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
$query=mysql_query("select barang_masuk.*,barang.*, supplier.* from barang_masuk, barang, supplier where barang_masuk.id_brg = barang.id_brg and barang_masuk.id_sup = supplier.id_sup");
while($lihat=mysql_fetch_array($query)){
	...
	$pdf->Cell(1.5, 0.9, $lihat['id_sup'], 1, 0, 'C');
	$pdf->Cell(3.5, 0.9, $lihat['nm_sup'], 1, 0, 'C');
	$pdf->Cell(1.5, 0.9, $lihat['no_sj'], 1, 0, 'C');
	$pdf->Cell(1, 0.9, $lihat['letak_brg'], 1, 0, 'C');
	$pdf->Cell(2, 0.9, $lihat['jenis_brg'], 1, 0, 'C');
	...
	$pdf->Cell(3, 0.9, date('d F Y', strtotime($lihat['tgl_masuk'])), 1, 1, 'C');
