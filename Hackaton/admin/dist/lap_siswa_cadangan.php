...
$pdf->Cell(25.5,0.7,"Laporan Data Siswa Cadangan",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(4, 0.9, 'NISN', 1, 0, 'C');
$pdf->Cell(7, 0.9, 'Nama Lengkap', 1, 0, 'C');
$pdf->Cell(5.5, 0.9, 'Email', 1, 0, 'C');
$pdf->Cell(3.5, 0.9, 'No. Handphone', 1, 0, 'C');
$pdf->Cell(2.5, 0.9, 'Rata-rata', 1, 0, 'C');
$pdf->Cell(2.5, 0.9, 'Hasil', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
$query=mysqli_query($conn, "SELECT user.*, nilai.rata, nilai.nisn, biodata.nisn, biodata.nama, biodata.status, biodata.id FROM user, biodata, nilai WHERE biodata.nisn = nilai.nisn AND biodata.id = user.id AND biodata.status = 'cadangan'");
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.9, $no , 1, 0, 'C');
	$pdf->Cell(4, 0.9, $lihat['nisn'],1, 0, 'C');
	$pdf->Cell(7, 0.9, ucwords($lihat['nama']), 1, 0,'C');
	$pdf->Cell(5.5, 0.9, $lihat[2],1, 0, 'C');
	$pdf->Cell(3.5, 0.9, $lihat[3], 1, 0,'C');
	$pdf->Cell(2.5, 0.9, $lihat['rata'], 1, 0,'C');
...
$pdf->Output("lap_siswa_cadangan.pdf","I");
