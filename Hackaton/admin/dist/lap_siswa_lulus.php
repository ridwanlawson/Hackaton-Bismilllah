...
$no=1;
$query=mysqli_query($conn, "SELECT user.*, nilai.rata, nilai.nisn, biodata.nisn, biodata.nama, biodata.status, biodata.id, biodata.pilihan, jurusan.id, jurusan.nama FROM user, biodata, nilai, jurusan WHERE jurusan.id = biodata.pilihan AND biodata.nisn = nilai.nisn AND biodata.id = user.id AND biodata.status = 'Lulus'");
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.9, $no , 1, 0, 'C');
	$pdf->Cell(3.5, 0.9, $lihat['nisn'],1, 0, 'C');
